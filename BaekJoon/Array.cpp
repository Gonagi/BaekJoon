	...
	try {
		for (int i = 0; i < 9; i++) {
			...
			if (input < 1 || input > 100)
				throw std::out_of_range("Input이 범위를 벗어남");
...
int main() {
	...
	try {
		std::cin >> A >> B >> C;
		if (A < 100 || A > 1000)
			throw std::out_of_range("A가 범위를 벗어남");
		if (B < 100 || B > 1000)
			throw std::out_of_range("B가 범위를 벗어남");
		if (C < 100 || C > 1000)
			throw std::out_of_range("C가 범위를 벗어남");
...
int main() {
	...
	try {
		std::cin >> N;
		if (N < 0 || N > 100)
			throw std::out_of_range("과목의 개수 'N'이 범위를 벗어남");

		for (int i = 0; i < N; i++) {
			std::cin >> score[i];
			if (score[i] < 0 || score[i] > 100)
				throw std::out_of_range("점수가 음수거나 100을 초과함");
			if (score[i] == 0)
				count++;
		}

		if (count == N)
			throw std::out_of_range("모든 점수가 0이다.");
...
int main() {
	...
	try {
		std::cin >> C;
		if (C < 1)
			throw std::out_of_range("테스트 케이스의 개수 'C'가 범위를 벗어남");
		
		for (int i = 0; i < C; i++) {
			std::cin >> N;
			if (N < 1 || N > 1000)
				throw std::out_of_range("학생 수 'N'이 범위를 벗어남");
			for (int j = 0; j < N; j++) {
				std::cin >> score[j];
				sum += score[j];
			}
